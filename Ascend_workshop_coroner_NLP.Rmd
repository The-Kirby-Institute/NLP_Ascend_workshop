...
rec <- recipe(Outcome ~ text, data = training %>% select(Outcome, text)) %>%
  ...
  step_tokenize(text,
                options = list(lowercase = F,
                               strip_punct = TRUE)) %>%
  step_tokenfilter(text, min_times = 1, max_times = Inf, max_tokens = 1000) %>%
  step_tfidf(text)
...
#doMC::registerDoMC(cores = parallel::detectCores(logical = FALSE)) #Parallel processing with the max cores available
